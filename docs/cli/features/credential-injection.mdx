...
> **macOS note:** The first time nono reads a keychain credential, macOS will show an access prompt. Click **Always Allow** to avoid repeated prompts on future runs. See [Storing Credentials → macOS](#macos) for install-specific options.
...
macOS will show a keychain access prompt the first time nono reads a credential. How to handle it depends on how you installed nono:

**Installed via GitHub Releases** — the binary is Apple-signed and notarized, so you can grant silent access upfront with the `-T` flag:

```bash
security add-generic-password -s "nono" -a "openai" -w "sk-..." -T "$(which nono)"
```

This allows nono to read the credential without any prompt.

**Installed via Homebrew** — Homebrew bottles are built on Homebrew's infrastructure without the original signing identity, so `-T` has no effect. Instead, run nono once and click **Always Allow** on the prompt. After that, no further prompts will appear.

The proxy loads credentials before the sandbox starts. If you click **Allow** (not Always Allow) or dismiss the prompt, the credential load fails silently — make sure to click **Always Allow**.
...
**Installed via GitHub Releases** — append `-T "$(which nono)"` when storing the credential to grant silent access:

```bash
security add-generic-password -s "nono" -a "openai_api_key" -w "sk-..." -T "$(which nono)"
```

**Installed via Homebrew** — `-T` has no effect (Homebrew bottles are not signed with the original identity). Run nono once and click **Always Allow** on the prompt instead.
