...
| `approval_fs` | object | Ceiling and approval route for exact, invocation-scoped filesystem grants requested at the shim boundary. |
...
### Approval-Gated Filesystem Requests (Prototype)

`approval_fs` lets a command request an exact file or directory grant for one invocation. It is a ceiling, not an ambient grant: the supervisor canonicalizes the proposal, checks it against `read_roots`, `write_roots`, and the resolved deny list, sends the normalized proposal to the selected approval backend, and only adds it to the fresh child sandbox after approval. A backend cannot grant a path outside the configured roots. The normalized proposal and approval decision are also recorded in the command-policy audit event.

Because macOS cannot widen a running Seatbelt sandbox or trap an arbitrary failed `open(2)` like Linux seccomp-notify, the caller must describe its intent before the child starts. The prototype carries that intent in the reserved `NONO_TOOL_SANDBOX_FS_REQUEST` environment variable:

```sh
NONO_TOOL_SANDBOX_FS_REQUEST='{"filesystem":[{"path":"/srv/data/input.json","access":"read","kind":"file"}]}' jq . /srv/data/input.json
```

The value is removed from the child environment. `access` is `read`, `write`, or `read_write`; `kind` is `file` or `directory`. Because Tool Sandbox write policies permit the normal read-modify-write workflow, `write` is normalized to `read_write` before the backend sees the proposal. Requested paths and ceiling roots must already exist, `/` cannot be granted, directory requests that contain a denied path are rejected, and each request is limited to 32 grants. Request an existing parent directory with `write` when the command needs to create a new file beneath it.

```jsonc
{
  "approval_backends": {
    "operator": { "type": "terminal" }
  },
  "approval_defaults": { "backend": "operator", "timeout_secs": 30 },
  "commands": {
    "jq": {
      "sandbox": {
        "approval_fs": {
          "read_roots": ["/srv/data"],
          "write_roots": ["/srv/results"]
        }
      }
    }
  }
...
Use the special `commands."*"` entry to mediate otherwise-unlisted external commands. Explicit command entries win. At startup, nono enumerates executable names and inode identities in trusted original-`PATH` directories so it can create the exact shim filenames required by normal shell lookup and block absolute-path bypasses. It does not read, hash, or classify every executable: those steps happen lazily when a particular wildcard shim is first invoked and are cached for that runtime. The inventory fails closed above 8,192 distinct command names.
...
```jsonc
{
  "command_policies": {
    ...
    "approval_defaults": { "backend": "operator" },
    "commands": {
      "mytoolA": {
        "sandbox": {},
        "can_use": ["*"]
...
The wildcard covers external executables reached through `PATH`; it cannot replace shell builtins, aliases, shell functions, or commands introduced into `PATH` after runtime preparation. A newly installed command requires a new nono session so its shim can be inventoried. The prototype rejects combining `commands."*"` with `open_urls`, because URL delegation and wildcard command capture would both need to own the reserved `open` shim name.
